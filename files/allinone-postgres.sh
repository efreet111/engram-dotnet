...
su postgres -c "pg_ctl -D '$PGDATA' -o '-c listen_addresses=127.0.0.1 -p $PG_PORT' -l /tmp/pg.log start" >/dev/null

# Esperar a que esté listo
for _ in $(seq 1 30); do
  if su postgres -c "pg_isready -h 127.0.0.1 -p $PG_PORT" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
if ! su postgres -c "psql -h 127.0.0.1 -p $PG_PORT -tAc \"SELECT 1 FROM pg_roles WHERE rolname='$PG_SUPERUSER'\"" | grep -q 1; then
  su postgres -c "psql -h 127.0.0.1 -p $PG_PORT -c \"CREATE ROLE $PG_SUPERUSER LOGIN SUPERUSER\"" >/dev/null
fi
if ! su postgres -c "psql -h 127.0.0.1 -p $PG_PORT -tAc \"SELECT 1 FROM pg_database WHERE datname='$PG_DATABASE'\"" | grep -q 1; then
  su postgres -c "createdb -h 127.0.0.1 -p $PG_PORT -O $PG_SUPERUSER $PG_DATABASE" >/dev/null
fi
