...
| 30 | ENG-447 | P0 | Bug | `InsertMutationBatchAsync`: transaction opened but no `NpgsqlCommand.Transaction=tx` — batch mutations auto-commit individually (atomicity broken) | Ready | M | ← ENG-440 audit | ~10 command sites in Apply* chain. Fix: pass `NpgsqlTransaction tx` to all Apply* methods |
