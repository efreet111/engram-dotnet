...
| ✓ | HU-010 | P1 | Feature | Deployment Profile System (`ENGRAM_PROFILE`: local/remote-server/offline-first/desktop) | ✅ Done | M | HU-010 | `docs/tasks/HU-001-HU-099/HU-010-deploy-profile-system.md` |
...
| 10 | HU-019 | P0 | Bug | Fix `ENGRAM_SYNC_TARGET` para perfil Desktop: código dice `"cloud"`, docs dicen `"desktop"`. Desktop es sync desktop↔laptop, no cloud centralizado. | ✅ Done | S | validación desktop profile 2026-08-19 | `docs/tasks/HU-001-HU-099/HU-019-fix-desktop-sync-target.md` — fix `DeployProfile.cs:93`, update test, fix `01-QUICK-START.md:295` |
| 11 | HU-020 | P1 | Feature | `engram doctor` perfil-aware: skip checks no aplicables por perfil, usa `ProfileValidator` para validar vars requeridas, muestra `Profile: X` en output | ✅ Done | M | análisis doctor 2026-08-19 | `docs/tasks/HU-001-HU-099/HU-020-doctor-profile-aware.md` — Profile enum en DiagnosticService, skip table, ProfileValidator integration |
| 12 | HU-021 | P0 | Bug | OpenCode error "missing key mcp.engram.enable" con perfil local: config activa `~/.config/opencode/opencode.json` falta `enabled: true` en `mcp.engram` | ✅ Done | XS | análisis opencode MCP 2026-08-19 | `docs/tasks/HU-001-HU-099/HU-021-fix-opencode-mcp-enable.md` — audit setup.sh/ps1, fix install.sh, documentar fix manual en INSTALLER-TROUBLESHOOTING |
| 13 | HU-022 | P0 | Bug | install.sh no descarga `libe_sqlite3.so` en Linux: `DllNotFoundException: Unable to load shared library 'e_sqlite3'` | ✅ Done | S | validación desktop profile + análisis install.sh 2026-08-19 | `docs/tasks/HU-001-HU-099/HU-022-fix-install-sh-native-libs.md` — install_release() ahora descarga libe_sqlite3.so y crea symlink e_sqlite3.so, test en test-install-wizard.sh |
| 14 | HU-023 | P2 | Feature | CLI profile management: `engram profile show` (muestra perfil + vars efectivas) y `engram profile set <profile>` (genera ~/.engram/.env con backup) | ✅ Done | M | análisis perfiles 2026-08-19 | `docs/tasks/HU-001-HU-099/HU-023-cli-profile-management.md` — profile show/set commands, --dry-run, --json, validación con ProfileValidator |
...
| 42 | ENG-474 | P0 | Bug | **HU-027: PostgreSQL session_id constraint on sync push**: mutations con `session_id` null/empty causan `NOT NULL REFERENCES sessions(id)` constraint violation en PostgreSQL. Sync push falla en batch y hace rollback atómico → retry loop infinito. Afecta a offline-first clients sin proyecto git-backed. | ✅ Done | S | ← HU-027 | `6c451fc` — `EnsureSessionAsync` helper + `COALESCE(NULLIF(@proj,''),'unknown')` + placeholders `obs-{EntityKey}` / `prompt-{EntityKey}`. 59 Postgres tests passed. T3 integration verified: null-session observation push accepted, placeholder session created. |
| — | **🔄 Multi-Project Sync (2026-08-11)** |
| 43 | ENG-488 | P1 | Feature | **HU-013: Multi-Project Sync Management**: per-project sync behavior (`silent-skip`/`fail-loud`), YAML config `~/.engram/sync-projects.dotnet.yml`, CLI interactive selector, per-server denylist. | ✅ Done | L | ← HU-013 | `docs/tasks/HU-001-HU-099/HU-013-multi-project-sync.md` — SyncProjectsConfig.cs, CLI commands `sync enroll`, tests SyncProjectsConfigTests, SyncBehaviorTests |
| 44 | ENG-489 | P1 | Feature | **HU-014: Smart Sync Triggers**: project-specific sync (not global), poll 30s only for modified projects, `engram sync push --project`, MCP `mem_save(sync_project=true)`. | ✅ Done | M | ← HU-014 | `docs/tasks/HU-001-HU-099/HU-014-smart-sync-triggers.md` — SyncManager updates, MCP trigger changes, tests SyncManagerSmartSyncTriggerTests |
| 45 | ENG-490 | P1 | Architecture | **RFC-006: Multi-Server Pull Deduplication**: cursor per server in `sync_state`, last-write-wins by `occurred_at`, tiebreaker by `server_id`, pull sequential with timeout. | 📝 Draft | M | ← RFC-006 | `docs/architecture/rfc/RFC-006-multi-server-pull-deduplication.md` — strategy defined, implementation pending |
| 46 | ENG-491 | P0 | Bug | **RFC-007/HU-016: Observations Schema Migration**: add `created_by TEXT` column to `observations` table in SQLite (consistency with `user_prompts` and `cloud_mutations`). | 📝 Draft | S | ← RFC-007 | `docs/architecture/rfc/RFC-007-observations-created-by-fix.md` — superseded by HU-016 |
...
| — | **Meta v1.1 — memoria semántica avanzada** |
| — | ENG-412 | P2 | Feature | Memory taxonomy & lifecycle (Decision, Insight, Transient, consolidation) | ✅ Done | L | ← PRD memoria semántica puntos #3, #10 | **Cerrado 2026-09-04**: status lifecycle (`active/deprecated/deleted`) + `mem_search --grouped` + `mem_decision_tree`. 1062 tests passing. Ver [summary](../.ai-work/eng-412-memory-taxonomy-lifecycle/summary.md). RFC-002-memory-taxonomy.md sigue pendiente (follow-up doc task, OQ-1). |
| — | ENG-413 | P2 | Feature | Smart token budget packer para queries | Icebox | M | ← PRD memoria semántica punto #4 | **Movido a Icebox 2026-08-27**: Los límites actuales (limit + truncamiento 300 chars) resuelven el problema práctico. Requiere ENG-412 (taxonomía) para criterio de "importancia". Reevaluar si el volumen de memorias crece o usuarios reportan costos excesivos. |
...
| 56 | ENG-476 | P1 | Feature | **Sync-on-demand (Push)**: cuando el sync está habilitado pero nadie lo arranca, las memorias están desincronizadas y el usuario no lo sabe. Trigger push después de cada mem_save, mem_update, mem_delete. Push inmediato de pendientes al arrancar MCP. Feedback de estado. | ✅ Done | M | ← sesión sync 2026-07-25 + ENG-476 analysis | PR #23 mergeado (`0258675`). Ver `.ai-work/eng-476-sync-on-demand/`. |
| 57 | ENG-477 | P1 | Feature | **Sync-on-demand (Pull)**: pull on-demand antes de mem_search y mem_context para traer memorias recientes de otros usuarios. Resuelve stale reads (30s delay). | Idea | S | ← ENG-476 analysis | Separado de ENG-476 para mantener ENGs enfocados. No hay data loss, solo inconveniencia (stale reads). Pull automático al arrancar MCP ya funciona. |
| 58 | ENG-478 | P1 | Doc | **Docker vanilla build**: diagnosticar error NuGet (`dev` no es SemVer válido) + alternativa para servidores sin acceso a `mcr.microsoft.com`. | ✅ Done | M | ← bug report usuario | Fix: `ARG ENGRAM_VERSION=0.0.0-dev` en Dockerfile. Nuevo: `Dockerfile.debian` (debian:12-slim + dotnet-install.sh). Docs: `docs/DOCKER-VANILLA.md` (334 líneas, 2 paths, troubleshooting). Ver `.ai-work/docker-vanilla-build-diagnosis/` |
| 59 | ENG-479 | P1 | Doc | **Docker runtime permissions**: fix SQLite Error 14 con volúmenes root-owned usando entrypoint + gosu. Documentación completa de variables de entorno y conexión PostgreSQL. | ✅ Done | M | ← bug report usuario | Entrypoint script + gosu, docs/DOCKER-VANILLA.md actualizado. Ver `.ai-work/eng-479-docker-runtime-permissions/` |
| — | **Developer Experience — Quick wins (2026-08)** |
| 60 | ENG-480 | P1 | Feature | **Quick-capture CLI**: `engram "<memo>"` — captura de memoria en un solo comando con smart defaults (auto-detect project, default type, auto-generate title). Reduce time-to-first-memory de 30s a 3s. | Idea | S | ← feature ideas 2026-08 | Ver [HU-050](../docs/tasks/HU-001-HU-099/HU-050-quick-capture-cli.md) |
| 61 | ENG-481 | P1 | Feature | **Git hooks integration**: `engram init` instala hooks de git (post-commit, prepare-commit-msg) para captura automática de memorias desde commits. Cada commit se convierte en memoria searcheable. | Idea | S-M | ← feature ideas 2026-08 | Ver [HU-051](../docs/tasks/HU-001-HU-099/HU-051-git-hooks-integration.md) |
| 62 | ENG-482 | P1 | Feature | **Dev-facing observability mejorado**: `engram stats` con recently created, oldest memories, by-type breakdown, storage size. Versión simplificada SIN schema migration (sin access tracking). | Idea | M | ← feature ideas 2026-08 | Ver [HU-052](../docs/tasks/HU-001-HU-099/HU-052-dev-observability.md) |
| — | **Developer Experience — Code-aware & Team features (future)** |
| 63 | ENG-483 | P2 | Feature | **Code-aware memory capture**: `engram watch <file>` + file watching para captura automática de memorias desde cambios de código. Complementa ENG-481 (git hooks). | Idea | L | ← feature ideas 2026-08 (IDEA-001) | Ver [HU-053](../docs/tasks/HU-001-HU-099/HU-053-code-aware-capture.md). File watching cross-platform es complejo. |
| 64 | ENG-484 | P2 | Feature | **Code-context query tools**: familia de MCP tools code-aware (`mem_recall_for_file`, `mem_recall_for_symbol`, etc.). Requiere schema evolution (ENG-416) para code metadata. | Idea | L | ← feature ideas 2026-08 (IDEA-002) | Ver [HU-054](../docs/tasks/HU-001-HU-099/HU-054-code-context-queries.md). Diferenciador clave: ningún competidor tiene code-aware recall. |
