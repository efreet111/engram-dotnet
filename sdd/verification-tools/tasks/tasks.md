...
- [x] 2.2 Create `src/Engram.Verification/CycleTracker.cs` — `GetOrCreateCycle/IncrementCycle/ResetCycle` via `IStore` + `topic_key=$"cycle-count/{changeName}"`; `Escalate` at `ENGRAM_VERIFICATION_MAX_CYCLES` (default 3)
